...
async function executeNotify(options: any): Promise<void> {
  ...
  const recentJudgments = obsidianAdapter
    .getRecentJudgments(hours)
    .filter(
      (j: any) => j && typeof j === "object" && (j.query || j.url || j.id),
    );
  ...
  // Group by query
  const byQuery = recentJudgments.reduce(
  ...
  recentJudgments.forEach((judgment, index) => {
    ...
    if (!judgment.filePath)
      console.log("   (warn: missing filePath in DB row)");
    console.log("");
  });
