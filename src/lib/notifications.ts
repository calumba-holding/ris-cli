...
export async function sendTelegramNotification(text: string, options: TelegramMessageOptions = {}): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn(`Failed to send Telegram notification: HTTP ${res.status} ${res.statusText} ${errText}`);
      return false;
    }
...
export function notifyNewJudgments(count: number, query?: string): void {
  ...
  void (async () => {
    const sent = await sendTelegramNotification(`🧾 Urteil Watchdog: ${message}`, { silent: count === 0 });
    if (sent) return;
...
export function notifyJudgmentList(judgments: ProcessedJudgment[], hours: number): void {
  const count = judgments.length;
  const header = `🧾 Urteil Watchdog: ${count} neue Urteile (letzte ${hours}h)`;
